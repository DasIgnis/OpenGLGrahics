...
const char* fsSourceLightingPhongColor =
	"#version 330\n"
	"out vec4 FragColor;\n"
	"in vec2 TexCoord;\n"
	"in Vertex {\n"
...
const char* vsSourceLightingPhongInt =
	"#version 330\n"
	"layout (location = 0) in vec3 position;\n"
	"layout (location = 1) in vec2 textureCoord;\n"
	"layout (location = 2) in vec3 normal;\n"
	...
	"	Vert.distance = length(lightDir);\n"
	"	vec3 normal   = normalize(Vert.normal);\n"
	"	vec3 lightDir2 = normalize(Vert.lightDir);\n"
	"	vec3 viewDir  = normalize(Vert.viewDir);\n"
	"	float attenuation = 1.0 / (light.attenuation[0] + light.attenuation[1] * Vert.distance + light.attenuation[2] * Vert.distance * Vert.distance);\n"
	...
	"	float RdotVpow = max(pow(dot(reflect(-lightDir2, normal), viewDir), material.shininess), 0.0);\n"
	"	Color += material.specular * light.specular * RdotVpow * attenuation;\n"
	"	gl_Position = transform.viewProjection * vertex;\n"
	"	TexCoord = textureCoord;\n"
	"}\n";

const char* fsSourceLightingPhongInt =
	"#version 330\n"
	"out vec4 FragColor;\n"
	"in vec2 TexCoord;\n"
	"in vec4 Color;\n"
	...
	"uniform sampler2D texture1;\n"
	"void main() {\n"
	"	FragColor = texture(texture1, Vert.texcoord) * Color;\n"
	"}\n";

const char* fsSourceLightingPhongIntColor =
	"#version 330\n"
	"out vec4 FragColor;\n"
	"in vec2 TexCoord;\n"
	"in vec4 Color;\n"
